...
def deployed_contract():
    ...
    nft_contract.setPublicSaleMintPrice(web3.toWei("0.2", "ether"))
...
def test_mint_separate_tx(deployed_contract):
    deployed_contract.publicMint(3, {"value": web3.toWei("1", "ether")})
    deployed_contract.publicMint(2, {"value": web3.toWei("1", "ether")})
...
#     for x in range(3):
#         deployed_contract.publicMint(5,
#                                      {"from": accounts[x], "value": web3.toWei("2", "ether")})
#     with reverts("dev: mint would exceed max token supply"):
#         deployed_contract.publicMint(3,
#                                      {"from": accounts[3], "value": web3.toWei("0.6", "ether")})


def test_cannot_mint_under_price(deployed_contract):
    with reverts("dev: msg.value too low"):
        deployed_contract.publicMint(2, {"value": web3.toWei("0.2", "ether")})
    with reverts("dev: msg.value too low"):
        deployed_contract.publicMint(1, {"value": web3.toWei("0.19", "ether")})
